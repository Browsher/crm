...
test('0026 inicializa dez empresas por autor usando último contato e ignora autor nulo', async () => {
  ...
  try {
    ...
    expect(await aplicar(banco.urlAdmin,PASTA_MIGRACOES)).toEqual({ ok:true, aplicadas:['0026_empresa_recentes.sql','0027_grupos_importacao.sql', '0028_comercial.sql', '0029_funil_comandos.sql', '0030_funil_avanco.sql', '0031_vendas_carteira_agenda.sql'] })
