...
test('atualização preserva dados anteriores, cria só ciclos de posse e não inventa vendas', async () => {
  ...
  try {
    ...
    expect(await aplicar(banco.urlAdmin,PASTA_MIGRACOES)).toEqual({ok:true,aplicadas:['0028_comercial.sql', '0029_funil_comandos.sql', '0030_funil_avanco.sql', '0031_vendas_carteira_agenda.sql']})
