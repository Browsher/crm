...
test('0022 atualiza banco povoado em 0021 preservando sessões e atribuindo versão inicial', async () => {
  ...
  try {
    ...
    expect(await aplicar(banco.urlAdmin, PASTA_MIGRACOES)).toEqual({ ok: true, aplicadas: ['0022_credencial_vigente.sql', '0023_empresa_cnae.sql', '0024_empresa_consulta.sql', '0025_fila_reservar.sql'] })
