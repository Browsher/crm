...
test('mutações têm retorno explícito, auditoria e FK impede remover grupo ou empresa vinculados', async () => {
  ...
  await expect(banco.sql('DELETE FROM grupo_importacao WHERE id=$1',[r.grupo_id])).rejects.toMatchObject({code:'23503'})
  await expect(banco.sql('DELETE FROM empresa')).rejects.toMatchObject({code:'23503'})
