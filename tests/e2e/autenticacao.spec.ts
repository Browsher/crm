...
test('gestor entra, acessa usuários e encerra a sessão', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Gerenciar empresas', exact: true })).toHaveAttribute('href', '/empresas')
  await expect(page.getByRole('link', { name: 'Gerenciar usuários', exact: true })).toHaveAttribute('href', '/usuarios')
