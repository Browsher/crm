...
test('gestor entra, acessa usuários e encerra a sessão', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Empresas', exact: true })).toHaveAttribute('href', '/empresas')
  await expect(page.getByRole('link', { name: 'Usuários', exact: true })).toHaveAttribute('href', '/usuarios')
