...
test('um texto por motivo', () => {
  ...
  expect(mensagemDeUsuario({ ok: false, motivo: 'alvo_inativo' })).toBe(
    'Não dá para definir senha de um usuário desativado. Reative antes.',
  )
  expect(mensagemDeUsuario({ ok: false, motivo: 'ja_nesse_estado' })).toBe(
    'Esse usuário já está nesse estado. Recarregue a lista.',
  )
