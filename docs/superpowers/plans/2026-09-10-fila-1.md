...
- **`fila_puxar` precisa de `#variable_conflict use_column`, e a razão foi medida na execução.** Os parâmetros de saída se chamam `empresa_id` e `reservado_ate`, iguais a colunas, e qualificar tudo (`f.`, `e.`, `ef.`) **não basta**: a cláusula `ON CONFLICT (empresa_id)` é inferência de índice e **só aceita nome de coluna nu** — não há qualificador possível ali. Com o padrão (`error`), a função falha com `column reference "empresa_id" is ambiguous`. A diretiva resolve a favor da coluna, e é segura aqui porque toda variável de verdade tem prefixo `v_` ou é alvo de `INTO`, que é sempre variável. Todas as outras referências continuam qualificadas, por leitura.
