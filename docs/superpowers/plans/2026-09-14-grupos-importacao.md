...
### Compatibilidade do teste de RESTRICT no CI

PR39 aberto. A primeira catraca, run34882798954 no7504df7, passou typecheck/lint/unitários, mas falhou1 de455 integrações: o teste exigia23503 para exclusão com RESTRICT. O PostgreSQL local17.11 retorna23503; o PostgreSQL18 do CI retorna23001. A diferença foi conferida nos ramos oficiais17/18 de ri_triggers.c e em probe local com as duas constraints reais.

Commitf23b380 altera somente o teste: exige major17/18 e o código exato dessa versão, nomes das duas constraints, schema/tabela e preservação integral do grupo, empresa e vínculo. Não aceita qualquer erro. Remover uma FK por vez em bancos temporários fez a respectiva asserção falhar; depois da restauração,17/17 testes do arquivo passaram, lint/typecheck passaram. Nenhuma migração/produto/harness alterado, demo preservada. O GREEN no PostgreSQL18 depende da próxima catraca; a rodada anterior não foi apresentada como aprovada.
