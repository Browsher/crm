...
- **Branch `fatia-empresas`, nunca commit na `main`.** A proteção real é
  `enforce_admins` no servidor (R-013), que recusa o push.
  **Não** havia aqui um `git branch --show-current` antes de cada commit: ele
  foi removido em 2026-09-09, depois de imprimir `main` e deixar o commit
  passar assim mesmo, porque estava encadeado com `&&`. Bilhete que parece
  catraca é pior que nada — ocupa o lugar da proteção de verdade.
...
- Produces: a tabela `empresa` com as sete colunas de conteúdo e o quarteto de
...
// com teste próprio. Para as sete colunas desta planilha, razão social com
...
describe('fase 1: a linha valida', () => {
  ...
  test('opcionais vazios viram null, nunca string vazia', () => {
    ...
    expect(a.aceitas[0]).toMatchObject({ nomeFantasia: null, contatoNome: null, email: null, cep: null })
  })
})
...
describe('modelo de importação', () => {
  ...
  test('o cabeçalho esperado tem sete colunas', () => {
...
2. Selecionar as colunas A até G inteiras → Formatar Células → **Texto**.
...
| 10 | Uma linha com CEP `00000000` | empresa entra; relatório diz "1 CEP não encontrado na base de 2024-07-08" |
